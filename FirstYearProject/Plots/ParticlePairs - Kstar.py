...
def get_masses_file(file):
    ...
    kaonPos = np.logical_and(particleTypes == 3,charge > 0)
    kaonNeg = np.logical_and(particleTypes == 3,charge < 0)
    piPos =   np.logical_and(particleTypes == 2,charge > 0)
    piNeg =   np.logical_and(particleTypes == 2,charge < 0)
    batchSplit = np.where(np.ediff1d(EventNumber, to_begin=0) != 0)[0]

    masses = []
    for start, end  in zip(batchSplit[:-1],batchSplit[1:]):
        particleA, particleB = np.meshgrid(np.where(kaonPos[start: end])+start,np.where(piNeg[start: end])+start)
        particleA = particleA.flatten()
        particleB = particleB.flatten()

        E = np.sqrt(Common.particles[3].m**2+momentumGlobal[particleA]**2) + np.sqrt(Common.particles[2].m**2+momentumGlobal[particleB]**2)
        P2 = (Px[particleA]+Px[particleB])*(Py[particleA]+Py[particleB])*(Pz[particleA]+Pz[particleB])
        masses.append(np.sqrt(E**2-P2))
        
        
    #for start, end  in zip(batchSplit[:-1],batchSplit[1:]):
    #    particleA, particleB = np.meshgrid(np.where(kaonNeg[start: end])+start,np.where(piPos[start: end])+start)
    #    particleA = particleA.flatten()
    #    particleB = particleB.flatten()
...
def main():
    np.seterr(all="ignore")
    p = Pool(4)
    #get_masses_file(Common.list_files("Data")[1])
    masses = np.concatenate(p.map(get_masses_file,Common.list_files("Data")))

    fig, ax = plt.subplots()
    plt.hist(masses, bins = 900, histtype = "step", range = (600,1500))

    ax.set_title('Mass of mother particle of (Kaon - Muon)')
